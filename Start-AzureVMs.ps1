...
param (
    ...
    [String]  $AzureCredentialAssetName = 'AzureLogin',
        
    [Parameter(Mandatory=$false)]
    [String] $AzureSubscriptionIdAssetName = 'subscriptionId',

    [Parameter(Mandatory=$false)] 
    [String] $ResourceGroupName = "Datacentre1"
)
...
{ 
	$VMs = Get-AzureRmVM -ResourceGroupName $ResourceGroupName | Where-object {PowerState -eq "VM Deallocated"}
}
else 
{ 
	$VMs = Get-AzureRmVM
